...
describe('POST /api/v1/brands', () => {
  ...
  it('creates a brand and its audit record for an authenticated operator', async () => {
    const { cookie, userId } = await authenticatedCookie();

    const response = await request(app.getHttpServer())
      .post('/api/v1/brands')
      .set('Cookie', cookie)
      .set('x-request-id', 'req-integration-create-brand')
      .send({ name: '  Natura  ' })
      .expect(201);

    expect(response.headers['x-request-id']).toBe(
      'req-integration-create-brand',
    );
    ...
    expect(
      ...
      userId,
      entityType: 'Brand',
      requestId: 'req-integration-create-brand',
    });
  });
  ...
  it('returns conflict for a duplicate name regardless of case', async () => {
    ...
    expect(
      await prisma.auditLog.count({ where: { action: 'BRAND_CREATED' } }),
    ).toBe(1);
  });
