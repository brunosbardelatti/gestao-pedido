...
type CreateCustomerFields = z.infer<typeof createCustomerSchema>;
...
const defaultValues: CreateCustomerFields = {
  name: '',
  cpf: '',
  phone: '',
  addressLine: '',
...
export function CreateCustomerForm(): React.JSX.Element {
  ...
  } = useForm<CreateCustomerFields>({
    resolver: zodResolver(createCustomerSchema),
    defaultValues,
  });

  async function submit(fields: CreateCustomerFields): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setRequestError(
          body.error?.message ?? 'Não foi possível cadastrar o cliente.',
        );
        return;
      }
      ...
      reset(defaultValues);
      setCreatedCustomer({ id: body.data.id, name: body.data.name });
      setFocus('name');
    } catch {
      ...
      {createdCustomer ? (
        ...
        >
          ...
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-4 gap-y-2">
            <span>Cliente {createdCustomer.name} cadastrado.</span>
            <Link
              href={`/customers/${createdCustomer.id}/edit`}
              aria-label={`Editar ${createdCustomer.name}`}
