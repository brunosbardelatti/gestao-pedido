...
Permitir que um usuário ativo acesse o sistema com login e senha.
...
- A senha deve ser validada com Argon2id.
- A autenticação deve criar uma sessão segura em cookie HTTP-only.
...
- Usuário ativo com credenciais válidas recebe sessão autenticada.
- Credenciais inválidas retornam erro sem revelar qual campo está incorreto.
- Cookie não pode ser acessado pelo JavaScript do navegador.
